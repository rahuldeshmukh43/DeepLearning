...
class Conv4d(nn.Module):
    # from https://github.com/timothygebhard/pytorch-conv4d/blob/master/conv4d.py
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
    ...
    def forward(self, input):
        ...
        for i in range(l_k):
            ...
            for j in range(l_i):
                ...
                frame_conv3d = \
                    self.conv3d_layers[i](input[:, :, j, :].view(b, c_i, d_i, h_i, w_i))
                #print(input.shape, b, c_i, d_i, h_i, w_i)
                #temp = input[:, :, j, :].view(b, c_i, d_i, h_i, w_i)
                #frame_conv3d = self.conv3d_layers[i](temp)
...
class BatchNorm4D(nn.Module):
    """
    Applies batch norm over a 6D input (a mini-batch of 4D with additional channel dimensions)
    ...
    def forward(self, X):
        ...
        Y, self.moving_mean, self.moving_var = batch_norm_4d(
        X, self.gamma, self.beta, self.moving_mean,
        self.moving_var, eps=1e-5, momentum=0.9)
        return Y
    
def batch_norm_4d(X, gamma, beta, moving_mean, moving_var, eps, momentum):
